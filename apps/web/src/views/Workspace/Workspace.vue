  ...
  <section class="workspace page-stack">
    ...
    <div class="dashboard-grid">
      <el-card class="recent" shadow="never">
        <template #header>
          <div class="card-header"><span>最近任务</span><el-button text @click="$router.push('/tasks')">查看全部 →</el-button></div>
        </template>
...
const overview = ref<SpaceOverview>({ total_tasks:0, succeeded:0, tasks_last_30d:0, knowledge_total:0, reuse_rate:0 });
...
.metric-card:hover { border-color: #9cb7a1; box-shadow: var(--shadow-sm); }.metric-card span,.metric-card small,.metric-card strong { display:block; }.metric-card span,.metric-card small { color:var(--text-muted); font-size:11px; }.metric-card strong { margin:8px 0 5px; font:700 28px var(--mono); }.dashboard-grid { display:grid; grid-template-columns:minmax(0,1.6fr) minmax(320px,.7fr); gap:14px; }.quick-row {
  width: 100%;
  display: grid;
...
.quick-row:hover { background: var(--surface-soft); padding-left: 4px; }.quick-row>span { color:var(--brand); font:700 11px var(--mono); }.quick-row strong,.quick-row small { display:block; }.quick-row small { color:var(--text-muted); margin-top:4px; }.quick-row>b { color:var(--brand); } @media(max-width:1000px){.workspace-hero,.dashboard-grid{grid-template-columns:1fr}.cards{grid-template-columns:repeat(2,1fr)}}
</style>
