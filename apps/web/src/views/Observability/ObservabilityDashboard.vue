<template>
  <section class="page-stack" v-loading="loading">
    ...
    <el-empty v-if="!loading && !summary && !error" description="暂无观测数据" />
    ...
    <div v-if="summary" class="ops-grid">
      <el-card shadow="never">
        <template #header>
          <div class="card-header">
            <span>实时运行面</span>
            <small class="muted">{{ summary.window }}</small>
          </div>
        </template>
      ...
      <el-card shadow="never">
        <template #header>
          <div class="card-header"><span>模型消耗</span></div>
        </template>
        <el-table :data="summary.models" empty-text="当前窗口暂无模型调用">
          ...
          <el-table-column label="Token" width="110">
            <template #default="{ row }">{{ number(row.tokens) }}</template>
          </el-table-column>
          <el-table-column label="成本" width="100">
            <template #default="{ row }">{{ money(row.cost_usd, 3) }}</template>
          </el-table-column>
        </el-table>
      </el-card>
    </div>
    ...
    <el-card v-if="summary" shadow="never">
      <template #header>
        <div class="card-header">
          <span>近期失败任务</span>
          <small class="muted">点击任务编号进入证据时间线</small>
        </div>
      </template>
      <el-table :data="summary.recent_failures" empty-text="当前窗口没有失败任务" class="clickable-table">
        <el-table-column label="任务" min-width="220">
          <template #default="{ row }">
            <router-link :to="`/task/${row.run_id}`">#{{ row.run_id }} {{ row.title }}</router-link>
          </template>
        </el-table-column>
        <el-table-column prop="error" label="错误根因" min-width="360" show-overflow-tooltip />
        <el-table-column label="发生时间" width="180">
          <template #default="{ row }">{{ date(row.occurred_at) }}</template>
        </el-table-column>
      </el-table>
    </el-card>
  </section>
...
const metrics = computed(() =>
  summary.value
    ? [
        {
          label: "任务成功率",
          value: percent(summary.value.slo.task_success_rate),
          hint: "已进入终态的任务",
          tone: summary.value.slo.task_success_rate >= 0.9 ? "good" : "warn",
        },
        {
          label: "首次测试通过率",
          value: percent(summary.value.slo.first_pass_rate),
          hint: "无需自动修复的验证",
          tone: summary.value.slo.first_pass_rate >= 0.75 ? "good" : "warn",
        },
        {
          label: "p95 执行耗时",
          value: duration(summary.value.slo.p95_run_duration_ms),
          hint: "任务端到端延迟",
          tone: "neutral",
        },
        {
          label: "错误率",
          value: percent(summary.value.slo.error_rate),
          hint: "API 与 Worker 错误",
          tone: summary.value.slo.error_rate <= 0.02 ? "good" : "danger",
        },
        {
          label: "模型成本",
          value: money(summary.value.slo.cost_usd),
          hint:
            summary.value.slo.cost_usd == null
              ? "Provider 用量尚未接入"
              : `预算 $${summary.value.slo.budget_usd.toFixed(2)}`,
          tone:
            summary.value.slo.cost_usd == null || summary.value.slo.cost_usd <= summary.value.slo.budget_usd
              ? "neutral"
              : "danger",
        },
      ]
...
async function load() {
  ...
  error.value = "";
  try {
    summary.value = await observabilityApi.summary(windowRange.value);
  } catch {
    error.value = "无法读取观测汇总，请确认当前空间权限及观测服务状态。";
  } finally {
    loading.value = false;
  }
}
...
.metric-card strong {
  display: block;
  margin: 10px 0 6px;
  font: 700 clamp(22px, 3vw, 32px) var(--mono);
}
