...
```bash
cp .env.example .env
# 修改 MYSQL_PASSWORD、MYSQL_ROOT_PASSWORD、JWT_SECRET、NEO4J_PASSWORD 和模型配置
...
Worker 的 checkpoint 位于共享卷 `/workspace/runs/.zhixiao-state`（由 `ZHIXIAO_STATE_DIR` 指定），因此容器使用只读根文件系统时仍可恢复运行。该目录属于运行状态，不得提交到 Git 或作为任务仓库制品发布。
...
- `local`：适合可信仓库与本地开发，速度快，依赖路径/命令策略。
